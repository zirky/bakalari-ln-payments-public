...
// 🔧 OPRAVA: QR kód stažen jako Base64 a vložen přímo do emailu (emailový klient neblokuje ext. obrázky)
async function sendVoucherEmail(lnurl, czkTotal, totalSats) {
  ...
  console.log(`DIAGNOSTIKA: Stahuji QR kód z: ${qrCodeUrl}`);

  // Stáhneme QR obrázek a zakódujeme jako Base64 – emailový klient nebude blokovat externí obrázky
  let qrCodeImgTag;
  try {
    ...
    console.log(`DIAGNOSTIKA: QR kód stažen (${mimeType}, ${qrResponse.data.byteLength} bytes).`);
    qrCodeImgTag = `<img src="data:${mimeType};base64,${base64}" alt="QR Code" style="width: 200px; height: 200px; border: 1px solid #ccc;" />`;
  } catch (e) {
    console.warn('⚠️ Nepodařilo se stáhnout QR kód:', e.message);
    qrCodeImgTag = `<p style="color:#888;">[QR kód nelze načíst – použij LNURL odkaz níže]</p>`;
  }
...
async function main() {
  ...
  try {
    ...
    let balanceToSave = newRunningBalance;
